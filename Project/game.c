...
bool check_for_engame(){
    if(hp1 == 0){
        ...
        write_string("winner", 100 ,300, 6 );
        write_string(name2, 100, 400, size2);
    ...
    if(hp2 == 0){
        ...
        memset(buffer,0,get_hres()*get_vres()*get_bytes());
        drawSprite(endgame, endgame->x,endgame->y); 
        drawXpm(get_winner_xpm(),200,300);
        //write_string("winner", 100 ,300, 6 );
        write_string(name1, 320, 500, size1);
        drawSprite(space, space->x, space->y+200);
        return true;
    }
    return false;
...
void keep_in_end(){
    if(hp1 == 0){
        drawSprite(endgame,endgame->x,endgame->y);
        drawSprite(player2wins,player2wins->x,player2wins->y);
    }
    if(hp2 == 0){
        drawSprite(endgame,endgame->x,endgame->y);
        drawSprite(player1wins,player1wins->x,player1wins->y);
    }
    return;
}
